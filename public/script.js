...
function initCalculaImc() {
    ...
    async function calculaImc(event) {
        ...
        // Verifique se todos os campos estão preenchidos e se o termo foi aceito
        if (isNaN(peso) || isNaN(altura) || !termoAceito || !genero || !listaAfazeres) {
        ...
        exibeImc(imc, resultado, genero, listaAfazeres);
    }
...
function exibeImc(imc, resultadoElement, genero, listaAfazeres) {
    if (resultadoElement) {
        ...
        resultadoElement.textContent = `Seu IMC é: ${imc.toFixed(2)} (${mensagem})`;
        run(consulta, mensagensAdicionais);
    }
}

async function run(consulta, mensagensAdicionais) {
    try {
        ...
        // Escolha uma mensagem adicional aleatória
        const mensagemAleatoria = mensagensAdicionais[Math.floor(Math.random() * mensagensAdicionais.length)];
        const nota = document.querySelector('.piada');
        nota.innerText = mensagemAleatoria;
        ...
        const resultado = document.querySelector('code');
        resultado.innerHTML += `<br>Desculpe, houve um problema ao obter a recomendação.`;
    }
}
...
function initThemeDark() {
    ...
    const newMoonIcon = `<svg id="moon-icon" xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-moon-fill" viewBox="0 0 16 16">
  <path d="M6 .278a.77.77 0 0 1 .08.858 7.2 7.2 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277q.792-.001 1.533-.16a.79.79 0 0 1 .81.316.73.73 0 0 1-.031.893A8.35 8.35 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.75.75 0 0 1 6 .278"/>
    ...
    function toggleTheme() {
        ...
        // Reattach event listener to the new moon icon
    ...
    // Adicionar o evento ao ícone de alternância do tema
    const moonIcon = document.getElementById('moon-icon');
    if (moonIcon) {
